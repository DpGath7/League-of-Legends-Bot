...
namespace LeagueBot.Game.Entities
{
    public class MainPlayer : ApiMember<GameApi>
    {
        ...
        public void processSpellToEnemyChampions()
        {
            ...
            //ASHE
            tryCastSpellToEnemyChampion("E");
            tryCastSpellToEnemyChampion("W");
            tryCastSpellToEnemyChampion("Q");
            tryCastSpellToEnemyChampion("R");
        }
        ...
        public void castUlt(String ult)
        {
            InputHelper.PressKey(ult);
        }
        ...
        public bool isGettingAttacked()
        {
            ...
            if (dmg != 0)
            {
                Logger.Write("Getting attacked!");
                return true;
            }
            else
            {
                Logger.Write("Not Getting attacked");
                return false;
            }
        }
        ...
        {
            if (playerHasRange("enemy"))
             {
                Point go = ImageValues.EnemyChampion();

                InputHelper.MoveMouse(go.X, go.Y);
                BotHelper.Wait(50);
                InputHelper.PressKey("LShiftKey");
                InputHelper.RightClick(go.X + 39, go.Y + 129);
                attackAnimation();
            }
        }
        ...
        {
            if (enemyCreepHealth() <= game.attackDamage)
            {
                Point go = ImageValues.EnemyCreepPosition();
                InputHelper.MoveMouse(go.X, go.Y);
                BotHelper.Wait(50);
                InputHelper.PressKey("LShiftKey");
                InputHelper.RightClick(go.X + 28, go.Y + 42);
                attackAnimation();
            }
        }
        ...
        {
            BotHelper.Wait((int)((1 / game.attackSpeed) * 1000));
        }

        public void followCreep()
        {
            Point go = ImageValues.AllyCreepPosition();
        ...
        public void moveAwayFromEnemy()
        {
            ...
            InputHelper.RightClick(go.X + 45, go.Y + 100);
        ...
        public void moveAwayFromCreep()
        {
            ...
            Random r = new Random();
            InputHelper.RightClick(970, 540);
            //InputHelper.MoveMouse(970, 540);
            //InputHelper.PressKey("A");
        ...
        public void tryCastSpellToEnemyChampion(String spell)
        {
            Point go = ImageValues.EnemyChampion();
        ...
        public void tryCastSpellToChampion(String spell)
        {
            InputHelper.PressKey(spell);
            BotHelper.Wait(50);
        }
